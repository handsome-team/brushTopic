<template>
  <view class="practice">
    ...
    <view class="selectsubject"
      ...
      <view class="select">
        <view class="selectindex"
              v-for="(item,xindex) in practiceList"
              :key='item.title'
              @click="selectindex(xindex)">
          {{xindex+1}}
        </view>
      </view>
    </view>
    ...
    <view class="topic"
      ...
      <view class="ask">
        <!-- 单选 -->
        <radio-group @change="radioChange('radio',$event)"
                     v-if="practiceList[index].type=='radio'"
                     :key='practiceList[index].id'>
          <label class="uni-list-cell uni-list-cell-pd"
                 v-for="(value,mindex) in practiceList[index].options"
                 :key="value.title">
            <view class="askItem">
              <radio :value="`${mindex}`" />
              {{value.title}}
        ...
        <radio-group @change="radioChange"
                     v-if="practiceList[index].type=='judge'"
                     :key='practiceList[index].id'>
        ...
        <!-- 多选 -->
        <checkbox-group @change="checkboxChange('checkbox',$event)"
                        v-if="practiceList[index].type === 'checkbox'"
                        :key='practiceList[index].id'>
          <label class="uni-list-cell uni-list-cell-pd"
                 v-for="(value,cindex) in practiceList[index].options"
                 :key="value.title">
            <view class="askItem">
          ...
          <button class="sureBtn"
                  @click="sureBtn('checkbox')">确认选项</button>
        </checkbox-group>
        <view class="tip"
              v-if="tip">
      ...
      <view class="topic-operation">
        <view class="collect"
              @click="tocollect(practiceList[index].id)">
          <image src="/static/practice/001.png"
                 v-if="!isShow"></image>
          <image src="/static/practice/005.png"
                 v-else></image>
          <view>收藏</view>
        </view>
        <view class="right"
  ...
  methods: {
    ...
    radioChange(type, e) {
      ...
      if (this.userAnswer == this.rightAnswer) {
        this.tip = false;
        let rightclude = this.rightSubject.includes(
          this.practiceList[this.index]
        );
        let wrongclude = this.errorSubject.includes(
          this.practiceList[this.index]
        );
